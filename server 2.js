  ...
  try {
    ...
    res.json = (payload) => {
      if (payload && payload.ok === true) {
        ...
        const explicitSourceStatus = typeof payload.sourceRecovered === 'boolean';
        const constantsOnly = payload.recoveryKind === 'constants-and-cfg-only' || (Array.isArray(payload.notes) && payload.notes.some((note) => /did not complete|constant pool.*not devirtual/i.test(String(note))));
        payload.sourceRecovered = explicitSourceStatus ? payload.sourceRecovered : Boolean(payload.beautified && !constantsOnly && !payload.loaderOnly && !payload.failed && !payload.protected);
        if (!payload.sourceRecovered) payload.recoveryPercent = Math.min(payload.recoveryPercent, 25);
        payload.sourceUrl = payload.sourceRecovered && payload.beautified ? createSourceLink(payload.beautified) : null;
        if (payload.sourceUrl) payload.notes = [...(Array.isArray(payload.notes) ? payload.notes : []), `View Source: ${payload.sourceUrl}`];
        payload.files = payload.sourceRecovered ? {
          source: 'View Source',
