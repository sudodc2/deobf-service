...
function assertOutputContract(data, expectedName, expectSource = true) {
  ...
  if (expectSource) {
    ...
    assert.equal(data.files.source, null);
    assert.equal(data.files.beautified, null);
    assert.equal(data.files.decoded, 'decoded_constants.lua');
    assert.equal(data.sourceUrl, null);
    assert.equal(data.sourceRecovered, false);
  }
...
async function endpointMatrix() {
  ...
  try {
    ...
    const regressionFile = process.env.WEAREDEVS_REGRESSION_FILE;
    if (regressionFile) {
      const fs = require('node:fs');
      const wrd = await post(port, fs.readFileSync(regressionFile, 'utf8'));
      assertOutputContract(wrd, 'WeAreDevs', false);
      assert.equal(wrd.recoveryKind, 'constants-and-cfg-only');
      assert.ok(wrd.recoveryPercent <= 25);
    }
  } finally {
    server.kill('SIGTERM');
  }
